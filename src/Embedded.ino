...
void setup() {
 ...
 s_1.attach(3);  // attaches the servo on pin 3 to the servo object
 s_2.attach(5);
 s_3.attach(6);
 s_4.attach(10);
 s_T.attach(11);
...
//      }
      ...
      for (int i = 0; i<5; i++) {
        a1 = Serial.read();
        M[i] = (int) a1;              //send x_high to rightmost 8 bits
        M[i] = M[i]*256;         //shift x_high over to leftmost 8 bits
        a2 = Serial.read();
        M[i] |= (int) a2;                 //logical OR keeps x_high intact in combined and fills in
      }
